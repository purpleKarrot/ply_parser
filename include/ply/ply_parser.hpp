...
public:
	...
	private:
		...
		typedef boost::mpl::inherit_linearly<scalar_types, boost::mpl::inherit<boost::mpl::_1, callbacks_element<boost::mpl::_2> > >::type callbacks;
	...
	struct list_property_definition_callback_type
	{
		...
		typedef boost::function<boost::tuple<list_property_begin_callback_type, list_property_element_callback_type, list_property_end_callback_type>(const std::string&, const std::string&)> type;
	};
	...
	private:
		...
		struct sequence_product: boost::mpl::fold<Sequence1, boost::mpl::vector0<>, boost::mpl::joint_view<boost::mpl::_1, boost::mpl::transform<Sequence2, pair_with<boost::mpl::_2> > > >
		{
		};
	...
	bool parse_header(std::istream& istream);
	bool parse_body(std::istream& istream);
...
inline bool ply::ply_parser::parse(std::istream& istream)
{
	return parse_header(istream) && parse_body(istream);
}
...
inline void ply::ply_parser::parse_list_property_definition(const std::string& property_name)
{
	...
	current_element_->properties.push_back(boost::shared_ptr<property>(new list_property<size_type, scalar_type> (property_name, boost::get<0>(list_property_callbacks), boost::get<1>(list_property_callbacks), boost::get<2>(list_property_callbacks))));
}
