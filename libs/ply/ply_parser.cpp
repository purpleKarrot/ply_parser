...
bool ply::ply_parser::parse_header(std::istream& istream)
{
	std::string line;
	line_number_ = 0;
	...
	while (std::getline(istream, line))
	{
		...
		else
		{
			...
			else if (keyword == "element")
			{
				...
				boost::shared_ptr<element> element_ptr(new element(name, count, boost::get<0>(element_callbacks), boost::get<1>(element_callbacks)));
...
bool ply::ply_parser::parse_body(std::istream& istream)
{
	std::string line;
	...
	{
		for (std::vector<boost::shared_ptr<element> >::const_iterator element_iterator = elements.begin(); element_iterator != elements.end(); ++element_iterator)
		{
			...
			for (std::size_t element_index = 0; element_index < element.count; ++element_index)
			{
				...
				for (std::vector<boost::shared_ptr<property> >::const_iterator property_iterator = element.properties.begin(); property_iterator != element.properties.end(); ++property_iterator)
				{
