...
int main(int argc, char* argv[])
{
  ...
  for (argi = 1; argi < argc; ++argi) {
    ...
    if ((short_opt == 'h') || (std::strcmp(long_opt, "help") == 0)) {
      ...
      std::cout << "\n";
      std::cout << "Report bugs to <" << OBJ_PACKAGE_BUGREPORT << ">.\n";
      return EXIT_SUCCESS;
    }

    else if ((short_opt == 'v') || (std::strcmp(long_opt, "version") == 0)) {
      std::cout << "obj2raw (" << OBJ_PACKAGE_NAME << ") " << OBJ_PACKAGE_VERSION << "\n";
      std::cout << "Copyright (C) 2007 " << OBJ_PACKAGE_AUTHOR << "\n";
      std::cout << "\n";
