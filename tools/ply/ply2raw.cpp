...
private:
	...
	std::vector<boost::tuple<ply::float32, ply::float32, ply::float32> > vertices_;
};
...
boost::tuple<boost::function<void()>, boost::function<void()> > ply_to_raw_converter::element_definition_callback(const std::string& element_name, std::size_t count)
{
	if (element_name == "vertex")
	{
		return boost::tuple<boost::function<void()>, boost::function<void()> >(boost::bind(&ply_to_raw_converter::vertex_begin, this), boost::bind(&ply_to_raw_converter::vertex_end, this));
	}
	else if (element_name == "face")
	{
		return boost::tuple<boost::function<void()>, boost::function<void()> >(boost::bind(&ply_to_raw_converter::face_begin, this), boost::bind(&ply_to_raw_converter::face_end, this));
	}
	else
	{
...
template<>
boost::function<void(ply::float32)> ply_to_raw_converter::scalar_property_definition_callback(const std::string& element_name, const std::string& property_name)
{
	if (element_name == "vertex")
	{
		if (property_name == "x")
...
template<>
boost::tuple<boost::function<void(ply::uint8)>, boost::function<void(ply::int32)>, boost::function<void()> > ply_to_raw_converter::list_property_definition_callback(const std::string& element_name, const std::string& property_name)
{
	if ((element_name == "face") && (property_name == "vertex_indices"))
	{
		return boost::tuple<boost::function<void(ply::uint8)>, boost::function<void(ply::int32)>, boost::function<void()> >(boost::bind(&ply_to_raw_converter::face_vertex_indices_begin, this, _1), boost::bind(&ply_to_raw_converter::face_vertex_indices_element, this, _1), boost::bind(&ply_to_raw_converter::face_vertex_indices_end, this));
	}
	else
	{
		return boost::tuple<boost::function<void(ply::uint8)>, boost::function<void(ply::int32)>, boost::function<void()> >(0, 0, 0);
	}
}
...
{
	vertices_.push_back(boost::tuple<ply::float32, ply::float32, ply::float32>(vertex_x_, vertex_y_, vertex_z_));
}
...
void ply_to_raw_converter::face_vertex_indices_element(ply::int32 vertex_index)
{
	...
	{
		(*ostream_) << boost::get<0>(vertices_[face_vertex_indices_first_element_]) << " " << boost::get<1>(vertices_[face_vertex_indices_first_element_]) << " " << boost::get<2>(vertices_[face_vertex_indices_first_element_]) << " " << boost::get<0>(vertices_[face_vertex_indices_previous_element_]) << " " << boost::get<1>(vertices_[face_vertex_indices_previous_element_]) << " " << boost::get<2>(vertices_[face_vertex_indices_previous_element_]) << " " << boost::get<0>(vertices_[vertex_index]) << " " << boost::get<1>(vertices_[vertex_index]) << " " << boost::get<2>(vertices_[vertex_index]) << "\n";
		face_vertex_indices_previous_element_ = vertex_index;
	}
...
bool ply_to_raw_converter::convert(std::istream& istream, const std::string& istream_filename, std::ostream& ostream, const std::string& ostream_filename)
{
	...
	ply::at<ply::float32>(scalar_property_definition_callbacks) = boost::bind(&ply_to_raw_converter::scalar_property_definition_callback<ply::float32>, this, _1, _2);
	...
	ply::at<ply::uint8, ply::int32>(list_property_definition_callbacks) = boost::bind(&ply_to_raw_converter::list_property_definition_callback<ply::uint8, ply::int32>, this, _1, _2);
